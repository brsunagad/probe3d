...
class DINO(torch.nn.Module):
    def __init__(
        ...
        self.checkpoint_name = f"{dino_name}_{model_name}_diy_sc"
        ...
        # adapter
        print(f"Loading AggregationNetwork")
        ckpt_dir = "diy_sc/ckpts/dino_spair_0300.pth"
        feature_dims = [768]
        ...
        pretrained_dict = torch.load(ckpt_dir, map_location="cpu")
        self.aggre_net.load_pretrained_weights(pretrained_dict)
        self.aggre_net.eval()
    ...
    def forward(self, images):
        ...
        for i, x_i in enumerate(embeds):
            ...
            if i == len(embeds) - 1:  # Only apply to last feature
                with torch.no_grad():
                    x_i = self.aggre_net(x_i)
            outputs.append(x_i)
