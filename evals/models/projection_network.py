...
""""
Module taken from https://github.com/Junyi42/GeoAware-SC/blob/master/model_utils/projection_network.py
"""
...
class AggregationNetwork(nn.Module):
    ...
    def load_pretrained_weights(self, pretrained_dict):
        ...
        if (
            "mixing_weights" in custom_dict
            and "mixing_weights" in pretrained_dict
            and custom_dict["mixing_weights"].shape
            != pretrained_dict["mixing_weights"].shape
        ):
            ...
            custom_dict["mixing_weights"][4] = torch.zeros_like(
                custom_dict["mixing_weights"][4]
            )
        else:
            custom_dict["mixing_weights"][:4] = pretrained_dict["mixing_weights"][:4]
        ...
        matching_keys = {
            ...
            if k in custom_dict and k != "mixing_weights"
        }
